...
@testset "average_sums" begin
    ...
    @test(
        isequal(average(g(ind(:k))*σ(2, 2, ind(:k))), g(ind(:k))*average(σ(2, 2, ind(:k))))
    )
    ...
    it_1 = findfirst(
        x->typeof((x))==SymbolicUtils.BasicSymbolic{IndexedAverageSum}, arguments(ADsum1)
    )
    it_2 = findfirst(
    ...
    @test isequal(
        sqa.SpecialIndexedAverage(
            average(σ(1, 2, ind(:i))), [(ind(:i), ind(:j))]
    ...
    @test isequal(
        simplify(Σ(Σ(σ(2, 1, ind(:i))*σ(1, 2, ind(:j)), ind(:i)), ind(:j), [ind(:i)])),
    ...
    @test isequal(
        ...
        sqa.insert_index(
            sqa.insert_index(σ(1, 2, ind(:i))*σ(1, 2, ind(:j)), ind(:i), 2), ind(:j), 1
        ),
    )

    @test isequal(
        sqa.IndexedAverageSum(g(ind(:i)), ind(:i), []), average(Σ(g(ind(:i)), ind(:i), []))
    )
