...
@testset "Formal operator expressions" begin
    ...
    @test 2 * (cA + sA) == 2 * cA + 2 * sA
    @test cA / 2 == (1 // 2) * cA
    @test cA // 2 == (1 // 2) * cA
    @test cA / unwrap(θ) == cA / θ
    @test cA // θ == cA / θ
    ...
    @test sort([cos(2 * A), cos(A)]) == sort([cos(A), cos(2 * A)])
    p2 = a * cA
    p3 = p2 * a
    @test sort([p3, p2]) == sort([p2, p3])
    @test sort([2 * cA, cA]) == sort([cA, 2 * cA])
