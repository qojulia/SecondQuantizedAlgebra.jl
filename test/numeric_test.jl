...
@testset "numeric conversion" begin
    ...
    @testset "Type stability — _ranges_position" begin
        import SecondQuantizedAlgebra: _ranges_position
        @inferred _ranges_position(2, 1, [1, 2])
        @inferred _ranges_position(1, 1, [3])
    end

    @testset "to_numeric with ranges — superradiant pulse" begin
        hc = FockSpace(:cavity)
        ...
        b = tensor(bc, [basis_a for _ in 1:order]...)
        ψc = fockstate(bc, 0)
        ψa = normalize(nlevelstate(basis_a, 1) + nlevelstate(basis_a, 2))
        ψ = tensor(ψc, [ψa for _ in 1:order]...)
        ranges = [1, 2]

        # Expected numeric operators
        a_ = LazyTensor(b, [1], (destroy(bc),))
        σ_(i, j, k) = LazyTensor(b, [1 + k], (transition(basis_a, i, j),))

        @test to_numeric(σ(1, 2, 1), b, ranges) == σ_(1, 2, 1)
        @test to_numeric(σ(2, 2, 2), b, ranges) == σ_(2, 2, 2)
        @test to_numeric(a, b, ranges) == a_
        @test to_numeric(a * σ(2, 2, 2), b, ranges) == σ_(2, 2, 2) * a_

        @test numeric_average(σ(2, 2, 2), ψ, ranges) ≈ 0.5
        @test numeric_average(average(σ(2, 2, 1)), ψ, ranges) ≈ 0.5
        @test numeric_average(average(a'a), ψ, ranges) ≈ 0.0
        @test numeric_average(average(a * σ(2, 2, 1)), ψ, ranges) ≈ 0.0
    end

    @testset "to_numeric with ranges — basis conversion" begin
        ...
        b_1 = ⊗([NLevelBasis(2) for _ in 1:N_n]...)
        b_2 = ⊗([FockBasis(3) for _ in 1:N_f]...)
        b_ = b_1 ⊗ b_2

        ai(i) = IndexedOperator(Destroy(h_, :a, 2), i)
        σi(i, j, k) = IndexedOperator(Transition(h_, :σ, i, j, 1), k)

        @test to_numeric(ai(1), b_, ranges) ==
            LazyTensor(b_, [5], (destroy(FockBasis(3)),))
        @test to_numeric(ai(2), b_, ranges) ==
            LazyTensor(b_, [6], (destroy(FockBasis(3)),))
        @test to_numeric(σi(1, 2, 4), b_, ranges) ==
            LazyTensor(b_, [4], (transition(NLevelBasis(2), 1, 2),))
    end
    ...
    @testset "LazyKet support" begin
        if isdefined(QuantumOpticsBase, :LazyKet)
            ...
            ψlazy = LazyKet(b, (ψc, (ψa for _ in 1:order)...))
            @test numeric_average(σ(2, 2, 2), ψlazy, ranges) ≈ 0.5
            @test numeric_average(average(σ(2, 2, 1)), ψlazy, ranges) ≈ 0.5
            @test numeric_average(average(a'a), ψlazy, ranges) ≈ 0.0
            @test numeric_average(average(a * σ(2, 2, 1)), ψlazy, ranges) ≈ 0.0
        end
    end
end
