...
@testset "Regression: leaf fast path respects name as site identity" begin
    ...
    @test isequal(commutator(a, b'), a * b' - b' * a)
    @test iszero(commutator(a, b))
    @test iszero(commutator(a', b'))
    ...
    hq = PauliSpace(:q)
    @test isequal(commutator(Pauli(hq, :σ, 1), Pauli(hq, :σ, 2)), 2im * Pauli(hq, :σ, 3))
    hn = NLevelSpace(:at, 2)
    @test isequal(
        commutator(Transition(hn, :σ, 1, 2), Transition(hn, :σ, 2, 1)),
        Transition(hn, :σ, 1, 1) - Transition(hn, :σ, 2, 2),
    )

    # An unresolved index pair is undetermined, so it still short-circuits to zero.
    hm = FockSpace(:m)
    am = Destroy(hm, :a)
    i = Index(hm, :i, 4, hm)
    j = Index(hm, :j, 4, hm)
