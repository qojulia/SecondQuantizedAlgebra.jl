...
SecondQuantizedAlgebra.get_indices(qqq)
...
@test isequal([i_ind,j_ind],sort(sqa.get_indices(average(σ(1,2,i_ind)) + 3 + average(σ(2,1,j_ind)))))
...
#
# @test isequal(g(1.1),sqa.SingleNumberedVariable(:g,1))
# @test isequal(IndexedVariable(:Ω,1.1,2.1),sqa.DoubleNumberedVariable(:Ω,1,2))
...
@test isequal(sqa._inconj(average(σ(2,1,1)*σ(2,2,2)*σ(1,2,1))),(average(σ(2,2,1)*σ(2,2,2))))
