...
using Test
import SecondQuantizedAlgebra: simplify, QMul, QAdd, QSym, QField, sorted_terms
...
@testset "simplify" begin
    ...
    @testset "Collect like terms" begin
        ...
        @test only(collect(result)).arg_c == 5
    end
    ...
    @testset "Remove zero terms" begin
        ...
        result = simplify(s)
        @test length(result) == 1
        @test only(collect(result)).arg_c == 3
    end
    ...
    @testset "a + a = 2a" begin
        ...
        result = simplify(s)
        @test length(result) == 1
        @test only(collect(result)).arg_c == 2
    end
    ...
    @testset "Transition: simplify applies composition" begin
        ...
        @test length(result) == 1
        @test only(collect(result)).args_nc[1].i == 1
        @test only(collect(result)).args_nc[1].j == 3
    end
    ...
    @testset "Pauli: simplify applies σⱼ²=I and product rule" begin
        ...
        @test only(collect(result)).args_nc[1].axis == 3
    end
