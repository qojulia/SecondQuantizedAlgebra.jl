...
The star of this example is `normal_to_symmetric`. Wigner-function
...
> rewrite the Heisenberg drift in symmetric order, take the `average`,
...
````
F * a + F * a' - J * a * b' - Δ * a' * a - J * a' * b - Δ * b' * b + (1//2)*U * a' * a' * a * a + (1//2)*U * b' * b' * b * b
````
...
````
factorize_meanfield (generic function with 1 method)
````
...
````
-F*im - (1//2)*⟨a⟩*κ + J*⟨b⟩*im + (U + Δ)*⟨a⟩*im - U*⟨a'⟩*(⟨a⟩^2)*im
````
...
The stochastic variables are the averages themselves. `make_time_dependent`
...
2-element Vector{Symbolics.Equation}:
 ...
 Differential(t, 1)(⟨b⟩(t)) ~ (0 + 1im)*J*⟨a⟩(t) + (0 + 1im)*(U + Δ)*⟨b⟩(t) - (1//2)*⟨b⟩(t)*κ + (0 - 1im)*U*(⟨b⟩(t)^2)*conj(⟨b⟩(t))
````
...
````
(3.7306961894099726, 21.486880674064192)
````
