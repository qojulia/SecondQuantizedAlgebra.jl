...
Hilbert subspace, kept distinct via `assume_distinct_index`.
...
`i ≠ j` ambiguous.  Calling `assume_distinct_index` records the
...
````
ω * σ_i₂₂ + ω * σ_j₂₂ + J * σ_i₁₂ * σ_j₂₁ + J * σ_i₂₁ * σ_j₁₂
````
...
`expand_completeness` projects to the ``\{\sigma^{ij} : (i,j) \neq
...
````
(J + ω) * σ_i₂₁ + (J + ω) * σ_j₂₁ - 2J * σ_i₂₁ * σ_j₂₂ - 2J * σ_i₂₂ * σ_j₂₁
````
...
expected {0, ω-J, ω+J, 2ω} = [0.0, 0.75, 1.25, 2.0]
...
````
g * a' * σ_i₁₁ - g * a' * σ_i₂₂ - g * a' * σ_j₁₁ + g * a' * σ_j₂₂
````
...
polariton + dark prediction:           [0.75, 0.9672, 1.5328]
...
`Σ` sums.  The dipole-dipole term becomes
...
````
Σ(j=1:Nsym)Σ(i=1:Nsym)(i≠j) J * σ_i₂₁ * σ_j₁₂
````
...
````
N = 2:  numeric single-excitation = [0.7672, 0.95, 1.3328]
...
N = 4:  numeric single-excitation = [0.75, 0.95, 0.95, 0.95, 1.55]
        theory                     = [0.75, 0.95, 0.95, 0.95, 1.55]   (ε_pol± = ω_c ± g√N, dark ×3 at ω - J)
