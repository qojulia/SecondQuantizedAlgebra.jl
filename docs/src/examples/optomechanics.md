...
````
((-(g^2)) / ωₘ + ω₀) * a' * a + ωₘ * b' * b - ((g^2) / ωₘ) * a' * a' * a * a
````
...
````
(g₂ + ω₀) * a' * a + ωₘ * b' * b + g₂ * a' * a * b * b + 2g₂ * a' * a * b' * b + g₂ * a' * a * b' * b'
````
...
````
  (n=0, m=0)  E_th = 0.0   E_num = 0.0   |Δ| = 0.0
