...
````
2(v^2) + 2u*v * a * b + (u^2 + v^2) * a' * a + 2u*v * a' * b' + (u^2 + v^2) * b' * b
````
...
````
v^2 - u*v * c_bog * d_bog + u^2 * c_bog' * c_bog - u*v * c_bog' * d_bog' + v^2 * d_bog' * d_bog
````
...
````
-u*v + u^2 * c_bog * d_bog - u*v * c_bog' * c_bog + v^2 * c_bog' * d_bog' - u*v * d_bog' * d_bog
````
...
````
2(u^2) - 4u*v + 2(v^2) + (u^2 - 2u*v + v^2) * c_bog * c_bog + (2(u^2) - 4u*v + 2(v^2)) * c_bog * d_bog + (2(u^2) - 4u*v + 2(v^2)) * c_bog * d_bog' + (2(u^2) - 4u*v + 2(v^2)) * c_bog' * c_bog + (u^2 - 2u*v + v^2) * c_bog' * c_bog' + (2(u^2) - 4u*v + 2(v^2)) * c_bog' * d_bog + (2(u^2) - 4u*v + 2(v^2)) * c_bog' * d_bog' + (u^2 - 2u*v + v^2) * d_bog * d_bog + (2(u^2) - 4u*v + 2(v^2)) * d_bog' * d_bog + (u^2 - 2u*v + v^2) * d_bog' * d_bog'
````
...
Wigner-Weyl symbol via `normal_to_symmetric`.  Applied to the
...
````
-ε - 2(v^2)*ε + ε * c_bog' * c_bog + ε * d_bog' * d_bog
````
