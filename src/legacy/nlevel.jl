...
function Base.:(==)(h1::T, h2::T) where {T <: NLevelSpace}
    return (h1.name == h2.name && h1.levels == h2.levels && h1.GS == h2.GS)
end
...
struct Transition{H, S, I, A, M} <: QSym
    ...
    function Transition{H, S, I, A, M}(
        ...
        return new(hilbert, name, i, j, aon, metadata)
    end
end
function Transition(
...
function Transition(hilbert::NLevelSpace, name, i, j; metadata = NO_METADATA)
    return Transition(hilbert, name, i, j, 1; metadata)
end
...
function Base.adjoint(t::Transition)
    return Transition(t.hilbert, t.name, t.j, t.i, acts_on(t); t.metadata)
end
function Base.isequal(t1::Transition, t2::Transition)
    return isequal(t1.hilbert, t2.hilbert) &&
...
function Base.hash(t::Transition, h::UInt)
    return hash(t.hilbert, hash(t.name, hash(t.i, hash(t.j, hash(t.aon, h)))))
end
...
function (c::CallableTransition)(i, j)
    return Transition(c.hilbert, c.name, i, j, c.aon; c.metadata)
end

function Transition(hilbert, name, aon; metadata = NO_METADATA)
    return CallableTransition(hilbert, name, aon, metadata)
end
function Transition(hilbert::NLevelSpace, name; metadata = NO_METADATA)
    return CallableTransition(hilbert, name, 1, metadata)
end
