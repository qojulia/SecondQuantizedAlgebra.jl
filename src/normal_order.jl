"""
    normal_order(expr)
    normal_order(expr, h::HilbertSpace)

Apply normal ordering to a quantum expression: creation operators left of
annihilation operators. Applies all algebraic identities (Transition composition,
Pauli products) AND commutation-based reordering (Fock, Spin, PhaseSpace).
Also simplifies prefactors and collects like terms.

The `h` argument enables ground state rewriting for Transitions.
"""
...
"""
    normal_to_symmetric(expr::QAdd) -> QAdd

Convert a normal-ordered expression to symmetric (Weyl) ordering.
The input MUST be in normal-ordered form (a† left of a per site).

Uses the combinatorial formula: for each normal-ordered term `c · (a†)^m · a^n`,
the Weyl-ordered form is `c · Σ_k C(m,k) C(n,k) k! (-1/2)^k (a†)^{m-k} a^{n-k}`.

For multi-site terms, the conversion applies independently per site.
...
"""
    symmetric_to_normal(expr::QAdd) -> QAdd

Convert a symmetric (Weyl) ordered expression to normal ordering.
The input MUST be in Weyl-ordered form.

Uses the combinatorial formula with `+1/2` instead of `-1/2`.
Exact inverse of `normal_to_symmetric`: `symmetric_to_normal(normal_to_symmetric(x)) == x`.
"""
