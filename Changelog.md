...
- Hamiltonian-derived `Displace` constructors compute static equilibrium and bounded finite-harmonic frames for one-mode Fock and general quadratic phase-space references. The free homogeneous response is omitted, exact resonances are rejected, and the resulting frame can be applied directly to a larger Hamiltonian containing Kerr or other nonlinear terms.
