...
- Exactly solvable unitary transformations. A `UnitaryTransform` stores a unitary as its action on the generators of a site, built with `Displace`, `Rotation`, `Squeeze`, `Bogoliubov`, `RotatingFrame` or `DressedFrame`, composed with `*`, inverted with `inv`, and applied with `conjugate` (observables) or `transform` (Hamiltonians, which adds the gauge term of a time-dependent frame). Covers Fock, phase-space, Pauli/spin and N-level sites.
- `is_canonical(U)` certifies a transform against the defining algebra of every site it touches, with no matrix representation needed.
- `RotatingFrame(H0, t)` derives its map from the adjoint action of `H0` rather than by matching term shapes, so a beamsplitter, a harmonic `(x^2 + p^2)/2` and a parametric `a^2 + a'^2` are covered alongside the number-diagonal cases. Its gauge term is exactly `-H0`.
- `DressedFrame(H0)` is the static counterpart: any number of levels when the level energies and couplings are numeric, block by block when they are symbolic.
- `transform` collides with the `transform` exported by QuantumOptics and QuantumOpticsBase, so a session loading either must call `SecondQuantizedAlgebra.transform`.
- `Squeeze(a, r, ϕ, t)` accepts a time-dependent phase, and `Rotation(σ, W, t)` a time-dependent matrix. Both used to throw for want of a gauge term; the first now carries the su(1,1) closed form (including the `a'*a + 1/2` piece a moving phase adds), the second differentiates `W` entrywise into `im*Ẇ'W`. Passing a gauge explicitly as a fourth argument to `Rotation` still overrides it.
- `expim(x)` is the unit phase `exp(im*x)` held as one indivisible coefficient factor. Phases then multiply by adding exponents and conjugate by negating them, so `expim(x)*expim(-x)` cancels on its own and a time-dependent coefficient written this way composes with a rotating frame. Rotating frames are built from it, which makes an N-level frame carry one phase per level rather than one per transition: commensurate frequencies (`E1-E3 = (E1-E2) + (E2-E3)`) become exponent arithmetic, so `RotatingFrame` now covers an off-diagonal `H0` on any number of levels instead of only two, and a symbolic level-diagonal frame is exact rather than certified within a tolerance. Building a three-level frame went from 431 ms to 0.7 ms and checking it from 630 ms to 24 ms, and it now certifies where it previously returned a false negative. Frame coefficients display as `exp(im*ω*t)`, with conjugate pairs folded back to `cos`/`sin`. Static unit-modulus factors use it too, so `Rotation(a, θ)` and the `exp(im*ϕ)` of a squeeze certify exactly at `atol = 0` and declare no relation; real rotations (`cos(θ)*x + sin(θ)*p`, a spin rotation) and `cosh`/`sinh` keep the trigonometric spelling.
- `RotatingFrame` handles any number of coupled generators, not just phases and `2x2` blocks. When the block of the adjoint action is a real rate times a numeric Hermitian matrix plus a diagonal its generators share, it is diagonalized with `eigen` and each eigenvalue gets one phase atom. That covers a hopping chain or ring of identical modes with a symbolic frequency and coupling, a spin rotation about an arbitrary axis, and two coupled modes with a nonzero mode frequency (whose block is not traceless, so no `2x2` form applied either). Shared diagonals stay on their own phase, so the mode frequency still cancels out of a number-conserving observable. These frames cancel to rounding rather than exactly, since the eigenbasis is numeric.
- `canonicality_residuals` now also checks that the images of different sites commute, for a transform whose rules mix sites. Preserving each site's own algebra does not imply it: flipping the sign of one cross term of a two-mode squeeze leaves both same-site commutators right and breaks `[ã, b̃]`.
- `simplify` now folds `cos^2 + sin^2` and `cosh^2 - sinh^2` in coefficients. These are identities, so the fold needs no context; it runs on the parameter-polynomial tier at one to two orders of magnitude less cost than the CAS route, and unlike the CAS it still folds above degree 2.
- Coefficients whose trigonometric argument is composite (`cos(ω*t)`, from a rotation with a time-dependent angle) reduce about a third faster; `conjugate` through such a transform drops 15% and `is_canonical` 21%. This path stays the expensive one, roughly a hundred times a `RotatingFrame` conjugation, because a composite argument is not an atom and the coefficient never reaches the polynomial tier.
- `UnitaryTransform(G, θ)` builds the transform `exp(-im*θ*G)` from any generator whose adjoint action closes, and `UnitaryTransform(G, θ, t)` adds the gauge term `-∂ₜθ*G` for a `θ` that moves. This is the generic constructor the named ones are special cases of; `RotatingFrame(H0, t)` is now literally the case `θ = t`. `UnitaryTransform` is exported for it.
- `RotatingFrame` refuses a time-dependent `H0` instead of answering wrongly. `exp(-im*H0*t)` is not the propagator of a moving `H0` and `-H0` is not its gauge, but `RotatingFrame(ω*cos(t)*a'*a, t)` used to return `exp(-im*cos(t)*t*ω)*a` where the phase is `exp(-im*ω*sin(t))`, build the wrong gauge, and certify. The commuting case `H0 = f(t)*G` is covered exactly by passing the integral of `f` to the three-argument constructor.
- Driven frames. A drive makes the adjoint action affine rather than linear, which used to throw; the generators are now shifted by the c-numbers that homogenize it, so `RotatingFrame(η*(a + a'), t)` is a displacement linear in `t` and `RotatingFrame(ω*a'*a + η*(a + a'), t)` the displaced rotation, both exact. A drive with no such shift is resonant and throws naming the generators.
- A quadrature squeeze is reachable through the adjoint action: a Hermitian generator on an imaginary rate is a real scaling rather than a phase. Both directions land on one `exp` atom at opposite exponents, so the pair cancels natively; spelling the second as `exp(-y)` would mint an unrelated atom and the residual would never close.
- A coupled block that squares to a multiple of the identity is exponentiated exactly at any size, keeping its rate symbolic. That covers a detuned parametric amplifier in every regime, whose block has an unshared diagonal and is not Hermitian, so neither the `2x2` form nor the eigendecomposition applied. Above threshold, at threshold and below it all certify with no tolerance, and so does a fully symbolic `Δ*a'*a + g*(a^2 + a'^2)` whose sign cannot be decided.
- Uniform chains and rings now diagonalize in closed form instead of through `eigen`. A symmetric tridiagonal Toeplitz block uses the discrete sine transform and any circulant block uses the DFT, so a dressed basis on those comes out with exact entries and no rounding noise where the closed-form values are representable, and falls back to `eigen` otherwise. A frame keeps the numeric spectrum unless its weights are rational, since a radical coefficient cannot both stay exact and let a phase cancel against its conjugate.
- `conj` of a coefficient mixing a real factor with a phase now distributes instead of wrapping the whole expression in a `conj(...)` that never folded.
- Frame rates are no longer rooted numerically. The square root is carried as an unevaluated node, so a rate stays exact and substituting parameters that make it imaginary evaluates instead of raising a `DomainError`.
- Exactness follows the input: write `3//10` rather than `0.3` if you want a coefficient held exactly, since only values that do not round-trip through `Float64` stay rational. Integers and dyadic rationals such as `1//2` are already exact, and scaling a Hamiltonian to integers can collapse a frame onto a single phase atom.
- `DressedFrame` diagonalizes a symbolic block of three or more coupled levels when it is one real rate away from a numeric Hermitian matrix plus a shared diagonal: the eigenvectors are then numbers, which is all a dressed basis is, and the energies stay symbolic. A uniformly coupled chain or ring qualifies with both its spacing and its coupling free, and `RotatingFrame` follows through the same factorization.
- `Displace(a, A)` accepts an operator-valued amplitude commuting with the mode it displaces, which is the polaron (Lang-Firsov) class. The amplitude's own site is gated rather than covered: operators commuting with it pass through, and the rest throw, because their images are a displacement operator times themselves and not polynomials at all. Returning them untouched was wrong with no diagnostic, and `is_canonical` could not see it. The optomechanics example now runs the polaron transformation through the API and derives its Kerr term in code, instead of standing in a c-number for `a'*a` sector by sector.
- Collective N-level sites are supported by `Rotation(S, W)`, by `is_canonical`, and by `RotatingFrame` on the level diagonal. The rotation contraction is the N-level one unchanged, since it is linear in the atom sum and never uses the product law collective transitions do not obey; the residuals differ, testing the bracket law and the invariance of the atom number. That last one is not decoration: the contragredient map `S^{ij} -> -S^{ji}` passes every other residual and no unitary induces it.
- Indexed operator families. A transform built from an operator carrying a free index now covers the whole family as the same local transform at every site, so `conjugate` instantiates it wherever it meets one, inside a `Σ` or out of it, and `RotatingFrame(Σ(ω*a_i'*a_i, i), t)` is such a family. Indexed parameters follow the rules, relations included. A per-slot index (`i(3)`) still names one resolved site. Family rules must stay on the site they act on; a rule spanning two spaces is refused, because its instances would share the off-family site and break commutativity between them while passing every same-site test.
- The examples now carry out their frame changes through the API instead of by hand: `Bogoliubov`/`Squeeze` for the two-mode diagonalization, `RotatingFrame` for the Jaynes-Cummings interaction picture, `Displace` for the polaron transformation, a quadrature-pair `Squeeze` for the parametric-oscillator variances, and a beamsplitter `Rotation` for the resonant Dicke polaritons.
...
- `LinearAlgebra` moved from a weak to a hard dependency, for the eigendecomposition behind `DressedFrame`. Load time is unaffected: Symbolics already pulls it into the closure.
...
- `+` and `-` accept an unwrapped scalar `BasicSymbolic`, the form a `Number`-symtype parameter (`@variables α::Number`) takes because `Num <: Real` cannot wrap a complex symtype. `α * a` already worked; `a + α` threw a `MethodError`.

- A composite prefactor part is now parenthesized, so a trailing `im` and the following operators bind to the whole thing: `(cos(θ)^2 + sin(θ)^2)im * a` used to print as `cos(θ)^2 + sin(θ)^2im * a`, and `im*(ω + g)*a` as `g + ωim * a`.

- Display order of a sum is now reproducible when two terms differ only in a packed level or axis field. The print-path sort key omitted those, so the two axes of a spin triple (or two levels of one transition set) tied and fell back to dictionary iteration order. A polynomial coefficient whose scalars are all real and negative also factors out as ` - ` now, instead of printing `+ -sin(θ)`.
